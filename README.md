...
Install directly from [clawhub.ai/siyu-hu/agent-local-memory](https://clawhub.ai/siyu-hu/agent-local-memory), or via CLI:

```bash
clawhub install agent-local-memory
```
...
直接在 [clawhub.ai/siyu-hu/agent-local-memory](https://clawhub.ai/siyu-hu/agent-local-memory) 安装，或通过 CLI：
